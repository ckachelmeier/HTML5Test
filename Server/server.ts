...
class ChessLobby {
    ...
    private server;
    private wsServer;
    private numberConnected: number;
    ...
    constructor(port) {
        ...
        var that = this;
        this.wsServer.on('request', function (request) { that.newRequest(request); });
    }

    private closeRequest(reasonCode, description, index) {
    ...
    private newRequest(request) {
        ...
        this.numberConnected++;
        var that = this;
        var newConnection = { connection: connection, name: "" };
    ...
    private handleMessage(messageWrapper, index: number) {
        if (messageWrapper.type === 'utf8') {
            ...
            switch (messageType) {
                case "name":
                    ...
                    break;
                case "lobbyMessage":
                    var toSend = { userName: this.connections[index].name, message: messageValue };
                ...
                case "startGame":
                    ...
                    var chessServer: chess.Chess.ChessServerContext;
    ...
    private newUserName(userIndex: number, name: string) {
            ...
            for (var i = 0; i < this.maxConnections; i++) {
                if (this.isLoggedIn(this.connections[i])) {
                    ...
                    if (this.connections[i].inGame && this.connections[i].gameFull)
                        x.games[x.games.length] = { id: i, name: this.connections[i].name };
                }
            }
    ...
    private sendRefreshedGamesList() {
        var games = []
        ...
        for (var i = 0; i < this.maxConnections; i++){
            ...
            if (this.isLoggedIn(this.connections[i]) && this.connections[i].inGame && !this.connections[i].gameFull) {
                games[games.length] = { id: i, name: this.connections[i].name };
                console.log("here");
            }
        }
    ...
    private sendMessageToLoggedInUsers(message: string, except = -1) {
    ...
    private isLoggedIn(user: any): bool {
        if (user == null)
            return false;
        if (!user.connection)
            return false;
